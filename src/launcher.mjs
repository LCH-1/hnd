...
async function runUpdateCommand(argv, { env, stdout, stderr, fetchImpl = fetch, execPath, npmUpdate = applyNpmUpdate }) {
  ...
  if (action === 'help' || action === '--help' || action === '-h') {
    writeText(stdout, ko ? [
      ...
      '',
      '자동 업데이트:',
      '  hnd 실행 시 백그라운드에서 확인합니다. 기본 주기는 1시간입니다.',
      '  주기 변경: HND_UPDATE_INTERVAL_MINUTES=15 (5분~24시간으로 제한)',
      '  끄기: HND_DISABLE_AUTO_UPDATE=1',
      '  즉시 받으려면 hnd update apply를 사용합니다.',
      '',
      ...
      '',
      'Automatic updates:',
      '  Checked in the background whenever hnd runs. The default interval is 1 hour.',
      '  Change it: HND_UPDATE_INTERVAL_MINUTES=15 (clamped to 5 minutes-24 hours)',
      '  Turn it off: HND_DISABLE_AUTO_UPDATE=1',
      '  Use hnd update apply to fetch immediately.',
      '',
